"""Settings controls for locally validated templates and French CV examples."""
...
def render_local_reference_asset_form(
    service: ReferenceAssetStorageService,
    requirement: RequiredReferenceAsset,
) -> None:
    """Render one locally validated template or other canonical local asset."""
...
def _render_active_french_examples(
    ...
    if st.button("Remove from active examples", key="remove_french_example"):
        _change_french_example_state(service, asset_key=selected_key, restore=False)
...
def _render_removed_french_examples(
    ...
    if st.button("Restore example", key="restore_french_example"):
        _change_french_example_state(service, asset_key=selected_key, restore=True)
