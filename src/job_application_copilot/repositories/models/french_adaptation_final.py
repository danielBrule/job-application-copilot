"""Private persisted structured output from French review and rewrite."""
...
class FrenchAdaptationFinal(Base):
    """One reviewed French CV retained for one logical generation task."""

    __tablename__ = "french_adaptation_finals"
    __table_args__ = (UniqueConstraint("task_id", name="uq_french_adaptation_finals_task"),)
    ...
    french_adaptation_prompt_version: Mapped[int] = mapped_column(Integer, nullable=False)
    french_review_prompt_version: Mapped[int] = mapped_column(Integer, nullable=False)
