"""Private persisted structured output from CV-generation stage one."""
...
class CvGenerationBrief(Base):
    """One validated brief retained for one logical CV-generation task."""

    __tablename__ = "cv_generation_briefs"
    __table_args__ = (UniqueConstraint("task_id", name="uq_cv_generation_briefs_task"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    task_id: Mapped[int] = mapped_column(
        ForeignKey("background_tasks.id", ondelete="CASCADE"), nullable=False, index=True
    )
    target_cv_lane: Mapped[str] = mapped_column(String(255), nullable=False)
