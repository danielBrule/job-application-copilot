...
def test_rejects_missing_supported_lane(tmp_path: Path) -> None:
    ...
    with pytest.raises(RoutingConfigError, match="lane catalogue mismatch"):
...
def test_generates_and_resolves_three_materially_different_lanes(tmp_path: Path) -> None:
    ...
    for lane in (
        CvLane.APPLIED_AI_DEPLOYMENT_LEADERSHIP,
        CvLane.HEAD_OF_SOLUTIONS_ARCHITECTURE,
        CvLane.HEAD_OF_DATA_ANALYTICS_AI,
    ):
...
def test_packet_distinguishes_direct_context_and_vector_scopes(tmp_path: Path) -> None:
    ...
    packet = service.resolve(version, CvLane.HEAD_OF_SOLUTIONS_ARCHITECTURE).packet
...
def test_genai_scope_carries_conditional_guardrail_dependencies(tmp_path: Path) -> None:
    ...
    packet = service.resolve(version, CvLane.DATA_AI_VALUE_CREATION).packet
...
def test_regeneration_supersedes_prior_current_set(tmp_path: Path) -> None:
    ...
    assert service.resolve(version, CvLane.HEAD_OF_SOLUTIONS_ARCHITECTURE).summary == second
...
def test_routing_set_cannot_be_used_for_another_document_b_version(tmp_path: Path) -> None:
    ...
    with pytest.raises(DocumentBRoutingError, match="no current routing set"):
        service.resolve(second, CvLane.HEAD_OF_SOLUTIONS_ARCHITECTURE)
...
def test_new_document_b_version_with_unchanged_structure_generates_its_own_manifest(
    ...
    assert service.resolve(second, CvLane.HEAD_OF_SOLUTIONS_ARCHITECTURE).summary == second_summary
...
def test_rejects_additional_unsupported_lane(tmp_path: Path) -> None:
    ...
    with pytest.raises(RoutingConfigError, match="lanes"):
        load_document_b_routing_config(config_path)
